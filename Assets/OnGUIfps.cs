...
//	public Rect startRect = new Rect(10, 10, 75, 50); // The rect the window is initially displayed at.
	...
	void Start()
	{
		startRect = new Rect(Screen.width - 85, 10, 75, 50);
		StartCoroutine(FPS());
	}
	...
	{
//		GUI.Label(new Rect(Screen.width - 85, 0, startRect.width, startRect.height), sFPS + " FPS", style);
		GUI.Label(new Rect(0, 0, startRect.width, startRect.height), sFPS + " FPS", style);
		if(allowDrag)
			GUI.DragWindow(new Rect(Screen.width - 85, 0, Screen.width, Screen.height));
	}
}
